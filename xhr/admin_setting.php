...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'add_new_user') {
			if (empty($_POST['email']) || empty($_POST['username']) || empty($_POST['password']) || empty($_POST['gender']) || empty($_POST['designation']) || empty($_POST['first_name'])) {
				$result['message'] = 'All filed are required!';
				$status = 400;
			} else {
				$status = 200;
				$re_data     = array(
					...
					'email_code' => Wo_Secure(md5($_POST['username'] . '_' . rand(111, 999)), 0),
					'designation' => Wo_Secure($_POST['designation']),
					'department' => Wo_Secure($_POST['department']),
				...
				$is_team_leader = (isset($_POST['is_team_leader']) && $_POST['is_team_leader'] == 1) ? $_POST['is_team_leader'] : 0;
				if ($is_team_leader == true) {
					$re_data['is_team_leader'] = 1;
					$re_data['leader_id'] = 0;
				} else {
					$re_data['is_team_leader'] = 0;
					$leader_id = (isset($_POST['leader_id']) && $_POST['leader_id'] > 0) ? $_POST['leader_id'] : 0;
					$re_data['leader_id'] = $leader_id;
					
				}
				// print_r($re_data);
				$add_user = Wo_RegisterUser($re_data, true);
				if ($add_user) {
    ...
    if ($s == 'delete_email') {
        if (!empty($_GET['email_id']) && is_numeric($_GET['email_id']) && $_GET['email_id'] > 0) {
            $id = Wo_Secure($_GET['email_id']);
            $db->where('id', $id)->delete(T_EMAILS);
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'delete_career') {
        if (!empty($_GET['career_id']) && is_numeric($_GET['career_id']) && $_GET['career_id'] > 0) {
            $id = Wo_Secure($_GET['career_id']);
			$career	= $db->where('id', $id)->getOne(T_CAREER);
			@unlink($career->postPhotos);
            $db->where('id', $id)->delete(T_CAREER);
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'update_moderator_permission') {
        if (!empty($_GET['permission']) && !empty($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && in_array($_GET['permission_val'], array(
            ...
            if (!empty($user)) {
				...
				$user            = $db->where('user_id', Wo_Secure($_GET['user_id']))->where('admin', '0')->getOne(T_USERS);
				if (!empty($user)) {
					...
					if (!empty($user->permission)) {
						...
						if (!empty($wo['all_pages'])) {
							foreach ($wo['all_pages'] as $key => $value) {
								if (in_array($value, $wo['user_pages'])) {
									$permission[$value] = 1;
								} else {
									$permission[$value] = 0;
								}
