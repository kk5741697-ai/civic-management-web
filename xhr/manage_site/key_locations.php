...
if ($s == "key_locations") {
		...
		if (json_last_error() === JSON_ERROR_NONE) {
			...
			if (is_array($orders) && isset($orders['order'])) {
				...
				foreach ($orders as $order) {
					$db->where('id', $order['id'])->update(T_KEY_LOCATIONS, array('position' => $order['position']));
	...
	if ($a == "delete") {
		...
		if (empty($id)) {
			...
			$imgLink = $db->where('id', $id)->getOne(T_KEY_LOCATIONS);
			$delete = $db->where('id', $id)->delete(T_KEY_LOCATIONS);
	...
	if ($a == "edit") {
		...
		$distance = isset($_POST['distance']) ? Wo_Secure($_POST['distance']) : '';
		...
		$update_data = array(
			...
			'distance' => htmlspecialchars_decode($distance),
		...
		$update = $db->where('id', $id)->update(T_KEY_LOCATIONS, $update_data);
	...
	if ($a == "add") {
		...
		if (empty($name)) {
			...
			$data = array(
				'status' => 400,
				'message' => 'Distance name is required!'
			);
		} else if (empty($product)) {
			$data = array(
				'status' => 400,
			...
			$insert = $db->insert(T_KEY_LOCATIONS, array(
				'name' => htmlspecialchars_decode($name),
				'distance' => htmlspecialchars_decode($distance),
