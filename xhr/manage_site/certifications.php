...
if ($s == "certifications") {
		...
		if (json_last_error() === JSON_ERROR_NONE) {
			...
			if (is_array($orders) && isset($orders['order'])) {
				...
				foreach ($orders as $order) {
					$db->where('id', $order['id'])->update(T_CERTIFICATIONS, array('position' => $order['position']));
	...
	if ($a == "delete") {
		...
		if (empty($id)) {
			...
			$imgLink = $db->where('id', $id)->getOne(T_CERTIFICATIONS);
			$delete = $db->where('id', $id)->delete(T_CERTIFICATIONS);
	...
	if ($a == "edit") {
		...
		$update = $db->where('id', $id)->update(T_CERTIFICATIONS, $update_data);
	...
	if ($a == "add") {
		...
		if (empty($name)) {
			...
			$insert = $db->insert(T_CERTIFICATIONS, array(
				'name' => htmlspecialchars_decode($name),
