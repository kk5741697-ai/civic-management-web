...
if ($s == "awards") {
		...
		if (json_last_error() === JSON_ERROR_NONE) {
			...
			if (is_array($orders) && isset($orders['order'])) {
				...
				foreach ($orders as $order) {
					$db->where('id', $order['id'])->update(T_AWARDS, array('position' => $order['position']));
	...
	if ($a == "delete") {
		...
		if (empty($id)) {
			...
			$imgLink = $db->where('id', $id)->getOne(T_AWARDS);
			$delete = $db->where('id', $id)->delete(T_AWARDS);
	...
	if ($a == "edit") {
		...
		$update = $db->where('id', $id)->update(T_AWARDS, $update_data);
	...
	if ($a == "add") {
		...
		if (empty($name)) {
			...
			$insert = $db->insert(T_AWARDS, array(
				'name' => htmlspecialchars_decode($name),
